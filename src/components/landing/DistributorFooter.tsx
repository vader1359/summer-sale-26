...
export function DistributorFooter() {
  ...
  return (
    <section className="bg-[#fffdfa] pb-[60px] pt-[60px]">
      <div className="content-shell grid grid-cols-[380px_1fr] gap-6 px-0 max-xl:px-6 max-lg:grid-cols-1">
        <div className="relative aspect-[9/16] w-full max-w-[380px] overflow-hidden rounded-[8px] bg-transparent max-lg:max-w-[236px]">
          <iframe
            src="https://fast.wistia.net/embed/iframe/09rnmxq8yg?videoFoam=true&autoPlay=true&silentAutoPlay=allow&muted=true&volume=0&endVideoBehavior=loop&controlsVisibleOnLoad=false&playbar=false&fullscreenButton=false&smallPlayButton=false&volumeControl=false"
            title={t.home.distributorFooter.videoTitle}
            allow="autoplay; fullscreen"
            allowFullScreen
            className="block h-full w-full border-0 outline-none"
          />
        </div>
        ...
        <div className="flex min-h-[676px] items-start px-11 py-6 max-lg:min-h-0 max-md:px-0">
          <div className="w-full max-w-[784px]">
            <p className="mb-8 text-[18px] font-semibold uppercase leading-6 text-[#444]">
              {t.home.distributorFooter.label}
            </p>
            ...
            <h2 className="mb-8 max-w-[784px] text-[48px] font-normal leading-[56px] text-[#333] max-md:text-[36px] max-md:leading-[42px] max-sm:text-[30px] max-sm:leading-9">
              {t.home.distributorFooter.headlinePrefix}{" "}
              <span className="font-semibold text-[#ff3401]">USM</span>,{" "}
              <span className="font-semibold text-[#ff3401]">FRITZ HANSEN</span>{" "}
              {languageJoiner(language)}{" "}
              <span className="font-semibold text-[#ff3401]">LOUIS POULSEN</span>{" "}
              {t.home.distributorFooter.headlineSuffix}
            </h2>
            ...
            <div className="mb-[33px] flex flex-col gap-4">
              <InfoRow icon="pin">
                {t.home.distributorFooter.address}
              </InfoRow>
              <InfoRow icon="clock">
                {t.home.distributorFooter.hours}
              </InfoRow>
...
function languageJoiner(language: string) {
  if (language === "vi") return "và";
  if (language === "ko") return "및";
  return "and";
}
